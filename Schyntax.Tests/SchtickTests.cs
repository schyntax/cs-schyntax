...
namespace Schyntax.Tests
{
    ...
    public class SchtickTests
    {
        ...
        public void TaskWindow()
        {
            ...
            var window1 = schtick.AddTask(sch, (task, run) => { window1Run = true; }, lastKnownRun: fiveSecAgo.AddMinutes(-1), window: 60000);

            // this should not trigger an immediate event
            var window2Run = false;
            var window2 = schtick.AddTask(sch, (task, run) => { window2Run = true; }, lastKnownRun: fiveSecAgo, window: 60000);
