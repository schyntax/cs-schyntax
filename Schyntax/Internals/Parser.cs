...
namespace Schyntax.Internals
{
    public class Parser : ParserBase
    {
        ...
        private IntegerValueNode ParseIntegerValue(ExpressionType expressionType)
        {
            ...
            else if (IsNext(TokenType.MonthLiteral))
            {
                if (expressionType != ExpressionType.Months)
                    throw new SchyntaxParseException(
                        "Unexpected month literal. Month literals are only allowed in month expressions.",
                        Input,
                        Peek().Index);

                var tok = Advance();
                val.AddToken(tok);
                val.Value = MonthToInteger(tok.Value);
            }
            else
            {
                switch (expressionType)
                {
                    ...
                    case ExpressionType.Months:
                        throw WrongTokenException(TokenType.PositiveInteger, TokenType.MonthLiteral);
